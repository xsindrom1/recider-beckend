...
def get_recommendations(indices):
    print("All indices:", indices[0])  
    recommended_indices = indices[0][:5]  
    print("Selected indices:", recommended_indices) 

    recommended_data = df.iloc[recommended_indices]

    recommendations = [
        {'Title': row['Title'], 'Ingredients': row['Ingredients'], 'Steps': row['Steps']}
